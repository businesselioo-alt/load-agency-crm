...
const DARK_STATUS: Record<SlotStatus, { bg: string; border: string; text: string; ring: string; glow: string }> = {
  programmé:  { bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-400', ring: 'ring-emerald-500/30', glow: '0 0 12px rgba(16,185,129,0.15)' },
  en_attente: { bg: 'bg-amber-500/10',   border: 'border-amber-500/20',   text: 'text-amber-400',   ring: 'ring-amber-500/30',   glow: '0 0 12px rgba(245,158,11,0.15)' },
  annulé:     { bg: 'bg-red-500/10',     border: 'border-red-500/20',     text: 'text-red-400',     ring: 'ring-red-500/30',     glow: '0 0 12px rgba(239,68,68,0.12)' },
};
...
function DayCell({ day, dateString, isToday, isPast, isCurrentMonth, slots, activeColor, onAdd, onEdit }: {
  ...
  if (!isCurrentMonth) {
    cellClass += 'border-white/[0.03] bg-transparent opacity-20';
  } else if (isPast) {
    cellClass += 'border-white/[0.04] bg-white/[0.01] opacity-45';
  } else if (isToday) {
    cellClass += 'border-transparent bg-white/[0.03]';
  } else {
    cellClass += 'border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/[0.10]';
  }

  return (
    <div
      className={cellClass}
      style={isToday ? { borderColor: activeColor + '60', boxShadow: `0 0 0 1px ${activeColor}30, inset 0 0 20px ${activeColor}08` } : {}}
    >
      <div className="flex items-start justify-between">
        <span
          ...
          style={
            isToday
              ? { backgroundColor: activeColor, color: '#fff', boxShadow: `0 0 8px ${activeColor}60` }
              : { color: isPast ? '#444' : '#999' }
          }
        >
          {day}
        </span>
      ...
      {canAdd && (
        <button
          ...
          className="flex items-center justify-center gap-0.5 w-full py-1 text-[11px] text-[#444] hover:text-[#C9A84C] hover:bg-[#C9A84C]/5 rounded-lg transition-all"
        >
          <Plus size={10} /> SFS
        </button>
      )}
        ...
        return (
          <div key={monthKey}>
            <div className="flex items-center gap-3 mb-3">
              <h3 className="font-semibold text-white text-sm">{MONTHS_FR[month - 1]} {year}</h3>
            ...
            <div className="space-y-2">
              {monthSlots.map((slot) => {
                ...
                return (
                  <div key={slot.id} className="flex items-center gap-4 bg-[#111] rounded-xl border border-white/5 px-4 py-3 hover:border-white/10 transition cursor-pointer"
                    onClick={() => onEdit(slot)}>
                    <div className="text-center w-10 flex-shrink-0">
                      <p className="text-[10px] text-[#555] uppercase tracking-wide">{MONTHS_FR[d.getMonth()].substring(0, 3)}</p>
                      <p className="text-xl font-bold text-white leading-tight">{d.getDate()}</p>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-white">{slot.partnerAgency}</p>
                      <p className="text-xs text-[#666]">↳ {slot.partnerModel}</p>
                    </div>
                  </div>
                );
              })}
...
function AddModelForm({ onAdd, onClose, existingNames }: {
  ...
  return (
    <div className="absolute top-full left-0 mt-2 z-30 w-72 bg-[#111] rounded-2xl shadow-2xl border border-white/10 p-4">
      <div className="absolute -top-2 left-5 w-3 h-3 bg-[#111] border-l border-t border-white/10 rotate-45" />
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm font-semibold text-white">Nouvelle model</p>
        <button onClick={onClose} className="p-1 text-[#555] hover:text-white rounded-lg transition"><X size={15} /></button>
      </div>
      <form onSubmit={handleSubmit} className="space-y-3">
        {error && (
          <p className="text-xs text-red-400 bg-red-500/10 border border-red-500/20 px-2.5 py-1.5 rounded-lg">{error}</p>
        )}
        <div>
          <label className="block text-xs font-medium text-[#666] mb-1.5">Prénom *</label>
          <input ref={inputRef} type="text" placeholder="ex : Sophie" value={name}
            onChange={(e) => { setName(e.target.value); setError(''); }}
            className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder-[#444] outline-none focus:border-[#C9A84C] focus:ring-2 focus:ring-[#C9A84C]/20 transition" />
        </div>
        <div>
          <label className="block text-xs font-medium text-[#666] mb-2">Couleur</label>
          <div className="flex flex-wrap gap-2">
            {COLOR_PALETTE.map((hex) => (
              <button key={hex} type="button" onClick={() => setColor(hex)}
                ...
                style={{ backgroundColor: hex, boxShadow: color === hex ? `0 0 0 2px #111, 0 0 0 3px ${hex}` : 'none' }}>
                {color === hex && <Check size={12} className="text-white" strokeWidth={3} />}
              </button>
            ))}
          </div>
          <div className="mt-3 flex items-center gap-2 px-3 py-2 bg-white/5 border border-white/5 rounded-xl">
            <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: color, boxShadow: `0 0 6px ${color}80` }} />
            <span className="text-sm font-medium text-white">{name || 'Prénom'}</span>
          </div>
        </div>
        <div className="flex gap-2 pt-1">
          <button type="button" onClick={onClose}
            className="flex-1 py-2 text-sm text-[#888] border border-white/10 rounded-xl hover:bg-white/5 transition">Annuler</button>
          <button type="submit"
            className="flex-1 py-2 text-sm font-semibold text-black rounded-xl transition"
            style={{ backgroundColor: color }}>Ajouter</button>
        </div>
      </form>
    </div>
  );
    ...
    <div className="p-6 min-h-full">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 text-black rounded-xl text-sm font-semibold transition-all hover:brightness-110 shadow-lg"
      ...
      <div className="flex items-center gap-2 mb-5">
        <div className="flex gap-1 bg-[#111] rounded-2xl p-1.5 border border-white/5 overflow-x-auto flex-1">
          {models.map((m) => {
              ...
              <div key={m.name} className="flex items-center flex-shrink-0 group/tab">
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${
                    isActive ? 'text-black shadow-lg' : 'text-[#666] hover:text-white hover:bg-white/5'
                  } ${isAdmin ? 'pr-2' : ''}`}
                  style={isActive ? { backgroundColor: m.color, boxShadow: `0 2px 12px ${m.color}50` } : {}}
                >
                  <span className="w-2 h-2 rounded-full flex-shrink-0"
                    style={{ backgroundColor: isActive ? 'rgba(0,0,0,0.4)' : m.color, boxShadow: isActive ? 'none' : `0 0 6px ${m.color}80` }} />
                  {m.name}
                  {upcoming > 0 && (
                    <span className="text-xs px-1.5 py-0.5 rounded-md font-semibold"
                      style={isActive
                        ? { backgroundColor: 'rgba(0,0,0,0.25)', color: 'rgba(0,0,0,0.8)' }
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-3 rounded-2xl border text-sm font-medium transition-all ${
              showAddModel
                ...
                : 'bg-[#111] text-[#666] border-white/5 hover:border-white/10 hover:text-white'
            }`}
            ...
            return (
              <button key={v} onClick={() => setView(v)}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  isActive ? 'bg-white/[0.08] text-white shadow-sm' : 'text-[#555] hover:text-[#999]'
                }`}>
        ...
        <div className="bg-[#111] rounded-2xl border border-white/5 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
            <button onClick={() => setCurrentMonth(new Date(year, month - 1, 1))}
              className="p-2 rounded-xl hover:bg-white/5 transition text-[#555] hover:text-white">
              <ChevronLeft size={17} />
            </button>
            <div className="flex items-center gap-3">
              <h2 className="font-semibold text-white">{MONTHS_FR[month]} {year}</h2>
              {(year !== today.getFullYear() || month !== today.getMonth()) && (
                <button onClick={() => setCurrentMonth(new Date(today.getFullYear(), today.getMonth(), 1))}
                  className="text-xs font-medium px-2.5 py-1 rounded-lg transition hover:opacity-80"
          ...
          {/* Footer legend */}
          <div className="flex items-center gap-5 px-6 py-3 border-t border-white/[0.04] text-xs text-[#444]">
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded border" style={{ borderColor: activeColor, boxShadow: `0 0 4px ${activeColor}40` }} />
              Aujourd'hui
            </span>
            <span className="flex items-center gap-1.5">
        ...
        <div>
          <div className="flex items-center gap-2 mb-5 px-1">
            ...
            <span className="text-sm text-[#666]">
              {historySlots.length} SFS passé{historySlots.length > 1 ? 's' : ''} pour <strong className="text-white">{selectedModel}</strong>
            </span>
          </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#111] border border-white/10 rounded-2xl shadow-2xl w-full max-w-sm z-10 p-6">
            <div className="w-11 h-11 rounded-xl bg-red-500/10 border border-red-500/20 flex items-center justify-center mb-4">
              <X size={20} className="text-red-400" />
            </div>
            <h3 className="font-semibold text-white mb-1">Supprimer {confirmDelete} ?</h3>
            <p className="text-sm text-[#666] mb-6">
              Cette action supprimera <strong className="text-[#999]">{confirmDelete}</strong> et tous ses SFS planifiés.
            </p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmDelete(null)}
                className="flex-1 py-2.5 text-sm font-medium text-[#888] border border-white/10 rounded-xl hover:bg-white/5 transition">
                Annuler
              </button>
              <button onClick={() => deleteModel(confirmDelete)}
                className="flex-1 py-2.5 text-sm font-semibold text-white bg-red-500/80 border border-red-500/20 rounded-xl hover:bg-red-500 transition">
                Supprimer
              </button>
            </div>
          </div>
